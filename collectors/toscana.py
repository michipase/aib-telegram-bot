...
TOSCANA_WEB_URL = "https://www.lamma.toscana.it/meteo/rischio-incendi"
TOSCANA_API_URL = "https://geoportale.lamma.rete.toscana.it/rischiojson/rischiojson.php"
...
class ToscanaConnector(Connector):
    ...
    def fetch_source(self) -> dict[str, Any]:
        try:
            ...
            api_json = self.get_text(TOSCANA_API_URL)
            return {
                "api_json": api_json,
                "source_url": TOSCANA_API_URL,
            }

    def parse_bulletin(self, raw_source: Any) -> ToscanaBulletin:
        html, pdf_bytes, api_json, source_url = self._coerce_raw_source(raw_source)
        ...
        api_entries, api_published_at = self._parse_api_entries(api_json) if api_json else ([], None)
        entries = html_entries or pdf_entries or api_entries
    ...
    @classmethod
    def _coerce_raw_source(cls, raw_source: Any) -> tuple[str, bytes | None, str | None, str]:
        if isinstance(raw_source, dict):
            html = raw_source.get("html") or ""
            pdf_bytes = raw_source.get("pdf_bytes")
            api_json = raw_source.get("api_json")
            source_url = str(raw_source.get("source_url") or TOSCANA_URL)
            if isinstance(pdf_bytes, str):
                pdf_bytes = pdf_bytes.encode("utf-8")
            return str(html), pdf_bytes, str(api_json) if api_json is not None else None, source_url
        if isinstance(raw_source, bytes):
            return "", raw_source, None, TOSCANA_URL

        raw_text = str(raw_source)
        if raw_text.lstrip().startswith("({\"") and "\"results\":" in raw_text:
            return "", None, raw_text, TOSCANA_API_URL
        return raw_text, None, None, TOSCANA_URL

    @classmethod
    def _parse_api_entries(cls, api_payload: str) -> tuple[list[ToscanaRiskEntry], datetime | None]:
        payload = api_payload.strip()
        ...
        for row in results:
            ...
            zone_name = str(row.get("nome") or "").strip()
            if not zone_name:
                continue

            if published_at is None:
                published_at = cls._extract_published_at(str(row.get("data") or ""))

            fwi = cls._coerce_float(str(row.get("fwi_run0") or ""))
            codcom = str(row.get("codcom") or "").strip()
            zone_id = f"TOS-COM-{codcom}" if codcom else cls._normalize_zone_id(zone_name)
            entries.append(
                ToscanaRiskEntry(
                    zone_id=zone_id,
                    zone_name=zone_name,
                    risk_level=cls._risk_level_from_fwi(fwi),
                    indice=None,
                    fwi=fwi,
                )
            )
    ...
    @staticmethod
    def _risk_level_from_fwi(fwi: float | None) -> str:
        if fwi is None:
            return "ND"
        if fwi < 2:
            return "BASSO"
